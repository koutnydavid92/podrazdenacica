...
<head>
    ...
    <style>
        ...
        /* Lightbox */
        .lightbox {{ position: fixed; inset: 0; background: rgba(13,13,13,0.94); display: none; align-items: center; justify-content: center; z-index: 9999; padding: 2vh 2vw; cursor: zoom-out; }}
        .lightbox.open {{ display: flex; }}
        .lightbox img {{ max-width: 100%; max-height: 94vh; border-radius: 8px; box-shadow: 0 20px 80px rgba(255,0,255,0.25); }}
...
<body>
    ...
    <section id="pt-hero" class="page-hero">
        <div class="container">
            <div class="hero-text">
                <div class="hero-label">Číča Art Fest 2026 · poděkování partnerovi</div>
                {logo_block}
                <h1 class="hero-title">Díky, {e(p['name'])}.</h1>
                <p class="pt-lead">
    ...
    <section id="fotky" class="pt-section" style="background: rgba(255,255,255,0.02);">
        <div class="container">
...
{gallery}
    ...
    <div class="lightbox" id="lightbox" role="dialog" aria-label="Zvětšená fotka"><img src="" alt=""></div>
    ...
    <script>
        requestAnimationFrame(() => {{
            ...
            document.addEventListener('keydown', (ev) => {{ if (ev.key === 'Escape' && lb.classList.contains('open')) closeLb(); }});
